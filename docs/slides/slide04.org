...
# Time-stamp: <2023-10-13 13:09:56 mura>
:REVEAL:
...
** COMMENT stargazer
  ...
  #+begin_src R :exports results :results output html :tangle no
    ...
    stargazer::stargazer(tw_lm1, tw_lm2, tw_lm3, tw_lm4, tw_lm5,
                         ## column.labels=c("A","B","C","D","E"),
                         type="html",single.row=TRUE,ci=TRUE)
  #+end_src
  #+html: </font>
- 変数名の対応
...
- 数値実験により
  ...
  - 人工データを用いて標準誤差と真の誤差を比較する
    #+begin_src R :eval no
      ...
      summary(data_lm)$coef[,"Std. Error"] # 列名での指定
      summary(data_lm)$coefficients[,2]    # 列番号での指定．coef と省略してもよい
    #+end_src
