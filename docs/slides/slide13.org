...
# Time-stamp: <2024-01-09 15:26:30 mura>
:REVEAL:
...
#+begin_src R :file figs/13_aracf.png :exports results :results graphics :tangle no
  ...
  patch <- list()
  for(i in 1:K) {
    patch[[i]] <- autoplot(acf(ts_ar[,i], plot = FALSE),
                           colour = hue_pal()(K)[i],
...
#+begin_src R :file figs/13_maacf.png :exports results :results graphics :tangle yes
  #' MA(2)モデルの自己相関
  for(i in 1:K) {
    patch[[i]] <- autoplot(acf(ts_ma[,i], plot = FALSE),
                           colour = hue_pal()(K)[i],
...
#+begin_src R :file figs/13_arpacf.png :exports results :results graphics :tangle no
  #' AR(2)モデルの偏自己相関
  for(i in 1:K) {
    patch[[i]] <- autoplot(pacf(ts_ar[,i], plot = FALSE),
                           colour = hue_pal()(K)[i],
                           main = paste("AR series:", i))
  }
  do.call("wrap_plots", patch)
...
#+begin_src R :file figs/13_mapacf.png :exports results :results graphics :tangle no
  #' MA(2)モデルの偏自己相関
  for(i in 1:K) {
    patch[[i]] <- autoplot(pacf(ts_ma[,i], plot = FALSE),
                           colour = hue_pal()(K)[i],
                           main = paste("MA series:", i))
  }
  do.call("wrap_plots", patch)
...
#+begin_src R :file figs/13_armapacf.png :exports results :results graphics :tangle yes
  #' ARMA(2,1)モデルの偏自己相関
  for(i in 1:K) {
    patch[[i]] <- autoplot(pacf(ts_arma[,i], plot = FALSE),
                           colour = hue_pal()(K)[i],
                           main = paste("ARMA series:", i))
  }
  do.call("wrap_plots", patch)
