...
# Time-stamp: <2024-10-03 17:55:17 mura>
:REVEAL:
...
- データの一部
  #+begin_src R :exports none
    ...
    #' モデル式
    tw_formula1 <- temp ~ press
    tw_formula2 <- temp ~ solar
    tw_formula3 <- temp ~ press + solar
    tw_formula4 <- temp ~ press + solar + humid
    tw_formula5 <- temp ~ press + solar + cloud
