...
モデルの推定には モデルを指定して関数 `fabletools::model()` を用いる．
...
関数 `AR()`, `ARMA()` ともに， モデル式を指定しない場合には 既定値では情報量規準の一つであるAICc(修正AIC)を用いて次数が自動的に決定される．
:::
...
推定されたモデルから情報を取得するための補助的な関数には以下のものがある．
...
あてはめ値は関数 `fitted()/fitted.values()` で， 残差は関数 `residuals()/resid()` でも取得することが可能である．
:::

モデルを評価するために以下の関数が用意されている．
...
#' data: 推定された時系列モデル
#' 詳細は `?ggtimes::gg_tsresiduals` を参照
```

描画には関数 `ggtime::gg_tsdisplay()` が利用される
:::
...
tw_tsbl |> # 一部を切り出して視覚化する
    filter_index("2024-06-01" ~ "2024-07-31") |>
    autoplot(temp, colour = "red") +
    labs(title = "Temperature in Tokyo",
         x = "date", y = "temperature")
```
...
推定されたモデルを用いて予測を行うには， 関数 \`fable::forecast()\*\* を用いる．
...
使い方は以下のようになるが， 描画には関数 `fabletools::autoplot.fbl_ts()` が利用できる．
