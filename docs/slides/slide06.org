...
# Time-stamp: <2023-10-09 12:42:51 mura>
:REVEAL:
...
#+end_src
#+begin_src R :exports none
  setwd("~/Desktop/lectures/u-tokyo/autumn/template")
#+end_src
...
- 数値実験により主成分分析の考え方を確認しなさい
  - 以下のモデルに従う人工データを生成する
    #+begin_src R :eval no
      ...
      n <- 100 # データ数
      my_data <- data.frame(runif(n,-1,1) %o% a + rnorm(2*n, sd=0.3))
    #+end_src
  - 観測データの散布図を作成
  - 観測データから第1主成分負荷量を推定
    #+begin_src R :eval no
      prcomp(my_data) # 全ての主成分を計算する
      ahat <- prcomp(my_data)$rotation[,1] # 負荷量(rotation)の1列目が第1主成分
    #+end_src
  - 散布図上に主成分負荷量を描画
    #+begin_src R :eval no
...
- 射影による特徴量の構成
  ...
  観測データ \(\boldsymbol{x}_{1},\dots,\boldsymbol{x}_n\)
  の情報を最も保持する1変量データを構成
  \begin{equation}
    \boldsymbol{a}^{\mathsf{T}}\boldsymbol{x}_{1},
...
- 第1主成分とGram行列の固有ベクトルの関係を調べなさい
  ...
  - Gram 行列を計算し固有値・固有ベクトルを求める
    #+begin_src R :eval no
      ## 中心化を行う
      X <- scale(my_data, scale=FALSE) # help(scale) でオプション scale を確認
      ## Gram 行列を計算する
      G <- crossprod(X)
      ## 固有値・固有ベクトルを求める
...
#+reveal: split
- データの散布図
  #+begin_src R :file figs/06_pairs.png :exports results :results graphics :tangle yes
    ## データの視覚化
    plot(js_data, col="blue") # いくつかの変数は相関強い
  #+end_src
...
#+reveal: split
- データの箱ひげ図
  #+begin_src R :file figs/06_boxplot.png :exports results :results graphics :tangle yes
    boxplot(js_data, col="green") # 箱ひげ図．変数のばらつきに大きな違いがある
  #+end_src
...
- 正規化したデータ(の一部)
  #+begin_src R :exports results :tangle yes
    head(scale(js_data), 18)
  #+end_src

#+reveal: split
- 正規化したデータの箱ひげ図
  #+begin_src R :file figs/06_normalized_boxplot.png :exports results :results graphics :tangle yes
    boxplot(scale(js_data), col="green") # 箱ひげ図．変数のばらつきがない
  #+end_src
...
#+reveal: split
- 主成分得点の表示
  #+begin_src R :file figs/06_pcaplot.png :exports results :results graphics :tangle yes
