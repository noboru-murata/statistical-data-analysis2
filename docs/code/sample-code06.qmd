...
::: {callout-tip}
次週詳しく説明するが，関数 `ggfortify::autoplot()`を利用することもできる．
...
関数 `prcomp()` に列名を渡すともう少し簡略に記述できる．

```{r}
#| column: margin
js_pca <- js_data |>
    column_to_rownames("Pref") |> # 都道府県名を列名に変換
    select(where(is.double)) |> # 実数値の列を取得
    prcomp(scale. = TRUE) # 変数のばらつきを規格化
autoplot(js_pca, label = TRUE, label.repel = TRUE) 
```

さらに地方区分ごとに点の色を変えることもできる．
...
::: callout-tip
データフレームの特定の列のみ変換を行うには関数 `dplyr::across()` を利用する 関数を組み合わせることで様々な条件が表現できる． 具体的な例は `?dplyr::across` や `?tidyselect::where` を参照のこと． 関数 `base::scale()` は行列を返すため，そのまま用いると列名が変更される． 上記の例ではベクトルを作成する関数 `base::c()` と 無名関数を用いてこれを解消している． 名関数の代わりにラムダ式 `~c(scale(.))` などを用いることもできる
:::
