...
#+DATE: 
# Time-stamp: <2023-10-06 15:16:05 mura>
...
** R : 行列とベクトルの定義(書き加える)
- matrixの定義，ベクトルの定義
- データフレーム(tibble)がmatrixであることを確認
** R : 行列とベクトルの計算
...
- 気候 (気温,降雨,日射,降雪,風向,風速,気圧,湿度,雲量) \\
  に関するデータ(の一部)
  #+begin_src R :exports results :results output :tangle yes
    #' データの読み込み
...
- 観測値とあてはめ値の比較
  ...
  #+begin_src R :file figs/03_models.png :exports results :results graphics :tangle yes
    ...
    with(tw_est5,
         points(y,fitted.values,col="cyan",pch=17))  # 三角
    legend("bottomright",inset=.05, # 凡例の作成
           col=c("orange","green","blue","brown","cyan"), pch=c(17,15,21,23,17), 
