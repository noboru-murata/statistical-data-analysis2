...
#+DATE: 
# Time-stamp: <2021-10-01 16:25:43 mura>
:REVEAL:
...
# 早稲田大学 (再)
- 気象庁より取得した東京の気候データ
  #+begin_src R :exports results :tangle no
    ## データの読み込み
...
- 関連するデータの散布図
  ...
  #+begin_src R :file figs/03_pairs.png :exports results :results graphics :tangle no
    ## 関連データの散布図
...
- 観測値とあてはめ値の比較
  #+header: :width 720 :height 720 :res 100
  #+begin_src R :file figs/03_models.png :exports results :results graphics :tangle no
    ## 観測値とあてはめ値の比較
    if(Sys.info()["sysname"]=="Darwin"){par(family="HiraginoSans-W4")}
    with(TW.est1,
