...
# Time-stamp: <2023-10-05 16:34:36 mura>
:REVEAL:
...
- ~pcr_case_daily.csv~ から以下の条件を満たすデータを取り出しなさい
  ...
  - 2020年3月の各機関(~g,i~ は集計なので除く)の検査件数データ
...
#+reveal: split
- [[color:green][民間検査会社と医療機関の検査件数の推移]]
  #+begin_src R :exports none
    #' @exercise 折れ線グラフの描画
...
#+reveal: split
- [[color:green][機関(医療機関まで)ごとの月の検査件数の推移]]
  #+begin_src R :exports none
    #' @exercise 散布図の描画
