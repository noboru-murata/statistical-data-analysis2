...
# Time-stamp: <2024-01-08 17:14:47 mura>
:REVEAL:
...
#+begin_src R :file figs/12_aracf.png :exports results :results graphics :tangle no
  ...
  patch <- list()
  for(i in 1:K) {
    patch[[i]] <- autoplot(acf(my_ar(a = a, epsilon = rnorm(Tmax)),
                               plot = FALSE),
                           colour = hue_pal()(K)[i],
                           main = paste("Series:", i))
  }
...
#+begin_src R :file figs/12_maacf.png :exports results :results graphics :tangle no
  ...
  b <- c(0.438, 0.078) # MAの係数
  for(i in 1:K) {
    patch[[i]] <- autoplot(acf(my_ma(b = b, epsilon = rnorm(Tmax)),
                               plot = FALSE),
                           colour = hue_pal()(K)[i],
                           main = paste("Series:", i))
  }
...
#+begin_src R :file figs/12_armaacf.png :exports results :results graphics :tangle no
  ...
  b <- -0.5          # MAの係数
  for(i in 1:K) {
    patch[[i]] <- autoplot(acf(my_arma(a = a, b = b, epsilon = rnorm(Tmax)),
                               plot = FALSE),
                           colour = hue_pal()(K)[i],
                           main = paste("Series:", i))
  }
...
:PROPERTIES:
:reveal_background: #fef4f4
:ID:       DADF6958-51CA-4315-82A5-5589457C7441
:END:
- 関数 ggfortify::autoplot()
  #+begin_src R :eval no :tangle no
