...
tw_lm
tw_df <- model.frame(tw_lm) |>     # 推定に用いたデータフレームの抽出
...
関数 `base::summary()` の tidyverse 版として `package::broom` が用意されている． 関数 `stats::lm()` の結果を tibble 形式で表示することができる． 詳細は後述する．
...
```{r}
library(broom)
glance(adv_lm3)                                   # 全ての指標を tibble 形式で取得
glance(adv_lm3)[["r.squared"]]                    # 決定係数
glance(adv_lm3)[["adj.r.squared"]]                # 自由度調整済み決定係数
adv_lm3 |>                                        # パイプ演算子を用いた方法
    glance() |>
...
bind_rows(                                        # 3つのモデルの評価指標を結合
    ...
    glance(adv_lm2),
    glance(adv_lm3)) |>
    mutate(model = c("モデル1","モデル2","モデル3"), # モデルの名称
           .before = 1) |>                        # 先頭に付加
    select(model,r.squared,adj.r.squared)         # 必要な3列を選択
#' 列番号がわかっている場合は select(1:3) でも良い
```
:::
...
glance(tw_lm1)[["adj.r.squared"]]           # 自由度調整済み決定係数
glance(tw_lm2)[["adj.r.squared"]]           # model1より上昇
glance(tw_lm3)[["adj.r.squared"]]           # model2より上昇
```
