...
都道府県名や項目名を日本語に書き換えて整理する例．

```{r}
#| eval: false
js_data <-
    bind_cols( # 2つのデータフレームを結合
        read_csv(file = "data/prefecture.csv",   # 都道府県名と地方区分
                 col_select = c(2,4)) |>
        set_names("都道府県名","地方区分"),          # 列名
        read_csv(file = "data/japan_social.csv",
                 col_select = 2:6) |> 
        set_names("森林面積割合","農業算出額","人口割合","土地生産性","商品販売額") # 項目名
    )|> 
    mutate(地方区分 = as_factor(地方区分)) # 地方区分を出現順に因子化
```
:::
