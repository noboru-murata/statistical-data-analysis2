...
Monte-Carlo 実験を行うには関数`purrr::map()` を利用すればよい
...
```{r}
mc_num <- 3               # 実験回数
1:mc_num |> map(mc_trial) # リストとして出力される
```
...
診断プロットは1から6まで用意されており 標準では 1,2,3,5 がまとめて表示される． 個別に表示する場合は `autoplot(tw_lm6, which = 1)` のように指定する． 詳細は `?ggfortify::autoplot.lm` を参照せよ．
:::
