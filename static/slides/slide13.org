...
** 偏自己相関
   - AR(p)モデル: 
     ...
     - ラグ\(p\)の *自己相関係数* (特殊な解釈)
       #+begin_quote
       \(a_{1}=a_{2}=a_{p-1}=0\)のときの\(a_{p}\)
       #+begin_src latex
         \begin{equation}
           \mathbb{E}[X_{t}X_{t+p}]=a_{p}\mathbb{E}[X_{t}X_{t}]
           \;\Rightarrow\; \gamma(p)=a_{p}\gamma_{0}
...
** R: ARモデルの推定 ~ar()~ 
   - 定常ARモデルのあてはめ
     #+begin_src R :eval no
       ...
       acf(resid(est)) # 残差(esplioneの推定値に相当)がホワイトノイズか確認する
     #+end_src
...
** R: 関数 ~zoo()~
   - 時系列クラス ~zoo~ を作成する関数
     #+begin_src R :eval no
       ...
       zoo(x = NULL, order.by = index(x))
       ## x: ベクトル，行列
       ## order.by: 成分の目盛
       zoo(x, # データに日付の情報を付加する例 (Dateクラスで指定)
