...
``` r
hclust.hclust(tree, k = NULL, which = NULL, x = NULL, h = NULL,
            border = 2, cluster = NULL)
#' tree: stats::hclust() の返値
#' 詳細は 'stats::hclust.hclust' を参照
```
:::
...
-   標準化して上記と同様の計算を行いなさい．
-   関数 `cluster::daisy()` による標準化を用いて，関東の都県同士の距離を表示しなさい．
-   大阪と四国の間の距離を表示しなさい．
...
標準化したユークリッド距離とマンハッタン距離を計算する． 関数 `stats::dist()` 単体では標準化する機能はないので，関数 `base::scale()` を併用する． 関数 `cluster::daisy()` には標準化のオプションがあるので，これを利用する．
...
関数 `base::scale()` と `cluster::daisy()` では標準化の方法が異なることに注意する．
...
以下では関数 `cluster::daisy()` による標準化を用いる．
...
-   平均0，分散1に標準化したデータのユークリッド距離を用いて，群平均法による階層的クラスタリングを行いなさい
-   クラスタ数を5つとして分割を行いなさい
...
```{r}
if(Sys.info()["sysname"] == "Darwin") par(family = jp_font) # macOS用
plot(js_hclust,
     hang = -1, # ラベルを揃えて表示
     cex = 0.8, # 文字のサイズを調整
     sub = "", xlab = "", main = "")
