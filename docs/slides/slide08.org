...
# Time-stamp: <2025-11-17 22:01:02 mura>
:REVEAL:
...
#+begin_src R :exports results :results value scalar latex :tangle no
  ...
  bio_data |>
    slice(1:15) |>
    gt() |>
    tab_options(table.font.size = 9, latex.tbl.pos = "H") |>
...
#+reveal: split
#+begin_src R :file figs/bio-pca.png :exports results :results graphics :height 700
  autoplot(bio_data |> select(where(is.numeric)) |> prcomp(), # 主成分分析
           data = bio_data,
           colour = "class") 
#+end_src
#+caption: 主成分分析を用いて2次元に縮約した生検と良性・悪性の関係
#+name: fig:bio-pca
#+attr_html: height 100%
#+attr_latex: :width 0.6\linewidth
...
#+reveal: split
#+begin_src R :file figs/bio-lda.png :exports results :results graphics :height 700
...
bio_data |>
    ...
    facet_grid(class ~ .)
#+end_src
#+caption: 線形判別関数による判別関数値の分布
#+name: fig:bio-lda
#+attr_html: height 100%
#+attr_latex: :width 0.6\linewidth
[[file:figs/bio-lda.png]]
...
#+begin_src R :file figs/bio-pairs.png :exports results :results graphics
      bio_data |>
        select(!ID) |> # IDを除く
        ggpairs(upper = list(continuous = wrap("cor", size = 3.5)),
                diag = list(continuous = wrap("densityDiag", alpha = 0.7),
                            mapping = aes(colour = class)),
                lower = list(continuous = wrap("points", alpha = .7),
                             mapping = aes(colour = class))) +
...
#+reveal: split
#+begin_src R :file figs/bio-pca.png :exports results :results graphics :height 700
  bio_data |> select(where(is.numeric)) |> prcomp() |> # 主成分分析
...
#+begin_src R :file figs/bio-lda.png :exports results :results graphics :height 700
  ...
  bio_data |>
    mutate(x = predict(bio_lda)[["x"]]) |>
    ggplot(aes(x = x)) +
    geom_histogram(aes(fill = class), alpha = 0.7,
                   show.legend = FALSE) +
    facet_grid(class ~ .)
#+end_src
...
:END:
- 9,10月のデータの散布図
  #+begin_src R :file figs/08_pairs.png :exports results :results graphics :height 700
    #' 東京の気象データによる判別分析
    library(MASS) # パッケージの読み込み
    #' データの整理
    tw_data <- read_csv("data/tokyo_weather.csv")
...
#+reveal: split
- 線形判別 (2値)
  #+begin_src R :file figs/08_lda_value.png :exports results :results graphics :height 700
    #' 線形判別関数の推定
    tw_model <- month ~ temp + humid
...
#+reveal: split
- 線形判別 (2値)
  #+begin_src R :file figs/08_lda.png :exports results :results graphics :height 700
    #' 判別関数により予測されるラベルの図示
    range_x <- range(tw_subset[["temp"]])  # 気温の値域
    range_y <- range(tw_subset[["humid"]]) # 湿度の値域
    grid_x <- pretty(range_x, 100) # 気温の値域の格子点を作成
...
#+reveal: split
- 2次判別 (2値)
  #+begin_src R :file figs/08_qda.png :exports results :results graphics :height 700
      #' 2次判別関数を推定
...
#+reveal: split
- Fisherの線形判別 (多値)
  #+begin_src R :file figs/08_multi.png :exports results :results graphics :height 700
    #' 多値判別
    tw_subset3  <- tw_data |>
      filter(month %in% c(9,10,11)) |>
      select(temp, humid, month) |>
