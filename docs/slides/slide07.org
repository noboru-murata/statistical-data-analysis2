...
#+DATE: 
# Time-stamp: <2022-11-18 14:50:55 mura>
:REVEAL:
...
- 分析結果の評価を行う関数
  ...
  #+begin_src R :eval no
    ## データフレームを分析
    est <- prcomp(data = データフレーム) # データフレーム全部を用いる場合
    est <- prcomp( ~ 変数名1 + ... + 変数名p, data = データフレーム)
    ## 主成分負荷量と寄与率を確認する
    summary(est)
    ## 寄与率を図示する
    plot(est)
...
- 行列の積による表現
  ...
  #+end_src
  #+end_quote
  - 行列 \(G\) はデータ数\(\times\)2型の行列
  - 行列 \(H\) は変量の数\(\times\)2型の行列
