...
#+DATE: 
# Time-stamp: <2025-12-12 13:01:01 mura>
...
#+begin_src R :exports none
  ...
  js_data <-
    read_csv("data/japan_social2.csv",
             col_types = list(地方区分 = col_factor()))
#+end_src
#+begin_src R :eval no :exports none
  js_data |> View() # 左上ペインに表として表示
#+end_src
...
#+reveal: split
#+begin_src R :file figs/js_cluspairs.png :exports results :results graphics :height 700
  #' データの視覚化
  js_data |> # 散布図．いくつかの変数は相関強いことがわかる
    ggpairs(columns = 3:7,
...
#+end_src
#+caption: 散布図
#+name: fig:js_cluspairs
#+attr_html: height 100%
#+attr_latex: :width 0.6\linewidth
[[file:figs/js_cluspairs.png]]
...
#+reveal: split
#+begin_src R :file figs/js_clusplot.png :exports results :results graphics
  #' クラスタ分析
  js_data |> 
    column_to_rownames(var = "都道府県名") |> 
    select(where(is.double)) |> 
...
#+end_src
#+caption: 散布図上のクラスタ構造 (クラスタ分析の概念図)
#+name: fig:js_clusplot
#+attr_html: height 100%
#+attr_latex: :width 0.6\linewidth
[[file:figs/js_clusplot.png]]
...
- 都道府県別の社会生活統計指標を用いて以下を確認しなさい
  #+begin_src R :eval no :tangle no
    ...
    js_df <- read_csv("data/japan_social.csv") |>
      ...
      select(-Area) # 地方名は除く
  #+end_src
  - 標準化せずにユークリッド距離とマンハッタン距離の計算を行いなさい
  - 標準化して上記と同様の計算を行いなさい 
  - 関東の都県同士の距離を表示しなさい (daisyによる標準化を用いなさい)
  - 大阪と四国の間の距離を表示しなさい
