...
# Time-stamp: <2023-11-01 18:27:53 mura>
:REVEAL:
...
- 分析結果の評価を行う関数:
  ...
  - ~ggfortify::autoplot()~: 主成分得点の図示
  #+begin_src R :eval no :tangle no
    #' データフレーム 'toy_data' を分析
    toy_pca <- prcomp( ~ x1 + ... + xp, data = toy_data)
    #' 主成分負荷量と寄与率を確認する
    summary(toy_pca)
    #' 第1,2主成分得点で散布図を描く
    library(ggfortify)
...
- それぞれのデータにおいて
  ...
  - ~japan_social.csv~ の読み込み方の例
    #+begin_src R :eval no :tangle no
      js_data <- read_csv("data/japan_social.csv")
    #+end_src
  - ~MASS::UScereal~ の整理の仕方の例
    #+begin_src R :eval no :tangle no
      ## カテゴリカル変数が含まれるので以下のように処理すると良い
      str(MASS::UScereal) # 各変数の属性を確認する．factor/intが不要
...
#+begin_src R :eval no :exports none
  ...
  #' 主成分分析
  js_pca0 <- prcomp(js_data[-1]) # 正規化なし
  js_pca1 <- prcomp(js_data[-1], scale. = TRUE) # 正規化あり
  ...
  autoplot(js_pca0) # 第1,2主成分得点で散布図を描く
  #' 正規化した場合
  summary(js_pca1)
  js_pca1$rotation
  autoplot(js_pca1)
...
- 変数間の散布図
  ...
  #+begin_src R :file figs/07_pairs_raw.png :exports results :results graphics :tangle yes
    ## 散布図 (正規化なし)
...
- 変数のばらつきに大きな違いがある
  #+header: :width 720 :height 720 :res 100
  #+begin_src R :file figs/07_boxplot_raw.png :exports results :results graphics :tangle yes
...
- 主成分負荷量 (正規化なし)
  #+begin_src R :exports results :tangle yes
    js_pca <- prcomp(js_data, scale.=FALSE)
    js_pca$rot
  #+end_src
...
- 第1,2主成分得点の表示
  #+header: :width 720 :height 720 :res 100
  #+begin_src R :file figs/07_pcaplot12_raw.png :exports results :results graphics :tangle yes
    xrange <- 1.1*range(predict(js_pca)[,"PC1"])
    yrange <- xrange-mean(xrange)
    plot(PC2 ~ PC1, data=predict(js_pca),
	 type="n",
	 xlim=xrange, ylim=yrange)
    text(PC2 ~ PC1, data=predict(js_pca),
	 labels=row.names(js_data),
	 col="orchid", cex=1.0)
   #+end_src
...
- 第3,2主成分得点の表示
  #+header: :width 720 :height 720 :res 100
  #+begin_src R :file figs/07_pcaplot32_raw.png :exports results :results graphics :tangle yes
    plot(PC2 ~ PC3, data=predict(js_pca),
         type="n",
         xlim=yrange, ylim=yrange)
    text(PC2 ~ PC3, data=predict(js_pca),
         labels=row.names(js_data),
         col="orchid", cex=1.0)
  #+end_src
...
- [[color:gray][正規化されていない場合は変数の分散の影響を考慮]]
